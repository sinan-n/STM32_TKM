...
const uint8_t anode_patterns[10][7] = {
    {0, 0, 0, 0, 0, 0, 1}, // 0: All ON except G
    {1, 0, 0, 1, 1, 1, 1}, // 1: Only B and C are ON (0)
    {0, 0, 1, 0, 0, 1, 0}, // 2: A,B,D,E,G are ON
    {0, 0, 0, 0, 1, 1, 0}, // 3: A,B,C,D,G are ON
    {1, 0, 0, 1, 1, 0, 0}, // 4: B,C,F,G are ON
    {0, 1, 0, 0, 1, 0, 0}, // 5: A,C,D,F,G are ON
    {0, 1, 0, 0, 0, 0, 0}, // 6: All ON except B
    {0, 0, 0, 1, 1, 1, 1}, // 7: A,B,C are ON
    {0, 0, 0, 0, 0, 0, 0}, // 8: All ON
    {0, 0, 0, 0, 1, 0, 0}  // 9: All ON except E
};
...
/* USER CODE END Boot_Mode_Sequence_2 */
  ...
  /* USER CODE BEGIN 2 */
  int counter = 0;
  set_display_anode(counter); // Show '0' on startup
  ...
  while (1)
  {
      ...
      // Since we used Pull-Up, pressed = Low (RESET)
      if (HAL_GPIO_ReadPin(EXT_BTN_GPIO_Port, EXT_BTN_Pin) == GPIO_PIN_RESET)
      {
          // 1. Increment Counter
          counter++;

          // 2. Reset to 0 if we go past 9
          if (counter > 9) counter = 0;

          // 3. Update Display
          set_display_anode(counter);

          // 4. Debounce (Ignore signal noise for 250ms)
          HAL_Delay(250);

          // 5. Wait here until button is RELEASED (so it doesn't count twice)
